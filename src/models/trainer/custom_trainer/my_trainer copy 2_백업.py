        ...
        *,
        ...
        for batch in data_loader:
            ...
            with torch.set_grad_enabled(is_train):
                ...
                # 디버그용: 한 번만 찍고 싶으면 바깥에 플래그 하나 두셔도 됨
                for task_idx, logits in enumerate(outs):
                    y_task = y[:, task_idx].long()

                    B, C = logits.shape
                    y_min = int(y_task.min().item())
                    y_max = int(y_task.max().item())

                    if y_min < 0 or y_max >= C:
                        print(f"[DEBUG] task_idx={task_idx}")
                        print(f"        logits.shape = {logits.shape}")
                        print(f"        y_min = {y_min}, y_max = {y_max}")
                        # 여기서 바로 죽여버리면, CUDA 커널까지 안 내려가고
                        # 정확히 어느 태스크에서 문제가 있는지 확인 가능
                        raise RuntimeError("label out of range: y in [{}, {}], C={}".format(y_min, y_max, C))
