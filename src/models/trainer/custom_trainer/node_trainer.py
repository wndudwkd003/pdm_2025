...
class NODETrainer(BaseTrainer):
    ...
    def fit(
        ...
        return {
            ...
            "n_valid": 0 if df_valid is None else len(df_valid),
    ...
    def save(self, save_path: Path) -> Path:
        ...
        out_dir = save_path / "pytorch_tabular_model"
        self.tabular_model.save_model(str(out_dir))
        return out_dir

    def load(self, model_path: Path):
        out_dir = model_path / "pytorch_tabular_model"
        self.tabular_model = TabularModel.load_from_checkpoint(str(out_dir))
