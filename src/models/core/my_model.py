...
class MyModel(nn.Module):
    ...
    def forward(
        ...
        step_outputs, M_loss = self.tabular_encoder(x_flat)
        tab_tok = torch.sum(torch.stack(step_outputs, dim=0), dim=0)  # (B*S, n_d)
        tab_tok = tab_tok.view(B, S, self.n_d)             # (B, S, n_d)
        print("tab_tok 차원", tab_tok.shape)
...
if __name__ == "__main__":
    ...
    out = model.forward(tabular_dummy, image_dummy)
